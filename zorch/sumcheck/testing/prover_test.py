...
class StandardRoundTest(absltest.TestCase):
    ...
    def test_call_threads_state_transcript_msg(self) -> None:
        ...
        carry = prover.initial_claim(f[None], fnp.sum(f), rounds=3)
        carry, _, msg = _product_round(1)(carry, cheap_transcript(KB))
        self.assertEqual(msg.shape, (2,))
        self.assertEqual(carry.state.shape, (1, 4))  # one round consumed
        # The round binds its own challenge as it folds.
        self.assertEqual(int(carry.claim.index), 1)
...
class CompressedProductRoundTest(absltest.TestCase):
    ...
    def test_call_threads_state_transcript_msg(self) -> None:
        ...
        stacked = fnp.stack([a, b])
        carry = prover.initial_claim(stacked, fnp.sum(a * b), rounds=3)
        carry, _, msg = prover.CompressedProductRound(challenges=_CH)(
            carry, cheap_transcript(KB)
        )
        self.assertEqual(msg.shape, (2,))
        self.assertEqual(carry.state.shape, (2, 4))  # one round consumed
        self.assertEqual(int(carry.claim.index), 1)
