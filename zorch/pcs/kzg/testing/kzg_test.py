...
class QuotientAndEvalTest(absltest.TestCase):
    def test_remainder_is_evaluation(self) -> None:
        ...
        coeffs = fnp.array([3, 5, 2, 7], dtype=KB)
        z = fnp.array(4, dtype=KB)
    ...
    def test_degree_one(self) -> None:
        ...
        coeffs = fnp.array([9, 2], dtype=KB)
        z = fnp.array(5, dtype=KB)
        q, fz = _quotient_and_eval(coeffs, z)
        self.assertEqual(q.shape, (1,))
        self.assertEqual(int(q[0]), 2)
        self.assertEqual(int(fz), int(fnp.array(9, KB) + fnp.array(2, KB) * z))

    def test_constant_rejected(self) -> None:
        with self.assertRaises(ValueError):
            _quotient_and_eval(fnp.array([7], dtype=KB), fnp.array(1, dtype=KB))
...
class KzgRoundTripTest(absltest.TestCase):
    def setUp(self) -> None:
        ...
        self.coeffs = fnp.array([3, 1, 4, 1], dtype=SF)  # f(x) = 3 + x + 4x² + x³
        self.z = fnp.array(7, dtype=SF)
