...
class SingleMatrixCommitmentSchemeTest(absltest.TestCase):
    def test_commit_applies_sp1_domain_separator(self) -> None:
        ...
        self.assertTrue(bool(fnp.array_equal(raw_root, _SP1_RAW_ROOT_4X8)))

        # Independent recomputation of SP1's separator over the golden root.
        params = sponge.hash(fnp.array([2, 8], dtype=F))  # [log_height, width]
        expected = comp.compress(fnp.stack([raw_root, params]))
        self.assertTrue(bool(fnp.array_equal(committed, expected)))
        self.assertTrue(bool(fnp.array_equal(committed, _SMCS_COMMIT_4X8)))
    ...
    def test_commit_returns_commitment_and_prover_layers(self) -> None:
        ...
        _, _, smcs = _smcs()
        matrix = fnp.arange(32, dtype=F).reshape(4, 8)  # height 4 -> 3 layers
    ...
    def test_open_batch_returns_rows_and_sibling_path(self) -> None:
        ...
        _, _, smcs = _smcs()
        matrix = fnp.arange(32, dtype=F).reshape(4, 8)  # height 4 -> log_height 2
        _, layers = smcs.commit(matrix)
        indices = fnp.array([0, 3])

        rows, proofs = smcs.open_batch(indices, matrix, layers)

        self.assertEqual(rows.shape, (2, 8))
        self.assertTrue(bool(fnp.array_equal(rows[0], matrix[0])))
        self.assertTrue(bool(fnp.array_equal(rows[1], matrix[3])))
        ...
        self.assertTrue(bool(fnp.array_equal(proofs[0], layers[0][indices ^ 1])))
        self.assertTrue(bool(fnp.array_equal(proofs[1], layers[1][(indices >> 1) ^ 1])))
    ...
    def test_verify_rejects_tampered_row(self) -> None:
        ...
        matrix = fnp.arange(32, dtype=F).reshape(4, 8)
        commitment, layers = smcs.commit(matrix)
        rows, proofs = smcs.open_batch(fnp.array([1]), matrix, layers)
        tampered = rows[0] + fnp.ones(8, dtype=F)
    ...
    def test_verify_rejects_index_out_of_bounds(self) -> None:
        ...
        matrix = fnp.arange(32, dtype=F).reshape(4, 8)
        commitment, layers = smcs.commit(matrix)
        rows, proofs = smcs.open_batch(fnp.array([0]), matrix, layers)
    ...
    def test_verify_rejects_wrong_proof_length(self) -> None:
        ...
        _, _, smcs = _smcs()
        matrix = fnp.arange(32, dtype=F).reshape(4, 8)  # log_height 2
        commitment, layers = smcs.commit(matrix)
        rows, proofs = smcs.open_batch(fnp.array([0]), matrix, layers)
    ...
    def test_heap_digests_layout(self) -> None:
        ...
        _, _, smcs = _smcs()
        matrix = fnp.arange(64, dtype=F).reshape(8, 8)  # height 8 -> 15-node heap
        _, layers = smcs.commit(matrix)
        heap = smcs.heap_digests(layers)
        self.assertEqual(heap.shape, (15, 8))
        self.assertTrue(bool(fnp.array_equal(heap[0], layers[-1][0])))  # root at 0
        self.assertTrue(bool(fnp.array_equal(heap[7:], layers[0])))  # leaves last
    ...
    def test_prove_openings_matches_open_batch_siblings(self) -> None:
        ...
        _, _, smcs = _smcs()
        matrix = fnp.arange(64, dtype=F).reshape(8, 8)  # height 8, tree_height 3
        _, layers = smcs.commit(matrix)
        heap = smcs.heap_digests(layers)
        indices = fnp.array([0, 3, 5])
        ...
        for level in range(3):
            self.assertTrue(bool(fnp.array_equal(paths[:, level, :], proofs[level])))
...
class BindStructureTest(absltest.TestCase):
    def test_count_shape_mismatch_raises(self) -> None:
        _, _, smcs = _smcs()
        commitment = fnp.arange(8, dtype=fnp.uint32).view(F)
        with self.assertRaises(ValueError):
            smcs.bind_structure(
                commitment,
                fnp.array([1, 2, 3], dtype=F),
                fnp.array([1, 2], dtype=F),
            )
