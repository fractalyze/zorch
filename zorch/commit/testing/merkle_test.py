...
class MerkleTreeTest(absltest.TestCase):
    def test_commit_layer_shapes(self) -> None:
        _, _, tree = koalabear16_merkle()
        matrix = fnp.arange(32, dtype=F).reshape(4, 8)  # height 4
    ...
    def test_open_verify_roundtrip_reconstructs_root(self) -> None:
        ...
        for i in range(4):
            ...
            self.assertTrue(bool(fnp.array_equal(op.row, matrix[i])))
    ...
    def test_reconstruct_root_returns_committed_root(self) -> None:
        ...
        for i in range(matrix.shape[0]):
            op = tree.open(matrix, layers, i)
            self.assertTrue(bool(fnp.array_equal(tree.reconstruct_root(i, op), root)))
    ...
    def test_opening_is_registered_pytree(self) -> None:
        ...
        self.assertTrue(bool(fnp.array_equal(rebuilt.row, op.row)))
    ...
    def test_vmap_open_matches_per_index_open(self) -> None:
        ...
        tree, matrix, _, layers = _committed_4x8()
        indices = fnp.arange(matrix.shape[0])
        ...
        for q in range(matrix.shape[0]):
            ...
            self.assertTrue(bool(fnp.array_equal(batched.row[q], single.row)))
            for lvl, sib in enumerate(single.path):
                self.assertTrue(bool(fnp.array_equal(batched.path[lvl][q], sib)))
    ...
    def test_vmap_reconstruct_root_recovers_committed_root(self) -> None:
        ...
        self.assertTrue(
            bool(fnp.all(frx.vmap(lambda r: fnp.array_equal(r, root))(roots)))
        )
    ...
    def test_reconstruct_root_recovers_root_on_deeper_tree(self) -> None:
        ...
        _, _, tree = koalabear16_merkle()
        matrix = fnp.arange(128, dtype=F).reshape(16, 8)  # height 16 -> depth 4
        root, layers = tree.commit(matrix)
        indices = fnp.arange(matrix.shape[0])
        openings = frx.vmap(tree.open, in_axes=(None, None, 0))(matrix, layers, indices)
        roots = frx.vmap(tree.reconstruct_root)(indices, openings)
    ...
    def test_reconstruct_roots_batches_mixed_depths(self) -> None:
        ...
        _, _, tree = koalabear16_merkle()
        deep_m = fnp.arange(128, dtype=F).reshape(16, 8)  # depth 4
        shallow_m = fnp.arange(32, dtype=F).reshape(4, 8)  # depth 2
        ...
        rows = fnp.stack([op.row for _, _, op in legs])
        indices = fnp.array([i for _, i, _ in legs])
        paths = fnp.stack(
            [
                fnp.pad(fnp.stack(op.path), ((0, max_depth - len(op.path)), (0, 0)))
                for _, _, op in legs
            ]
        )
        valid = fnp.stack([fnp.arange(max_depth) < len(op.path) for _, _, op in legs])

        roots = tree.reconstruct_roots(rows, indices, paths, valid)
        self.assertTrue(bool(fnp.all(roots == fnp.stack([r for r, _, _ in legs]))))
        for k, (_, idx, op) in enumerate(legs):  # identical to the per-call path
            self.assertTrue(
                bool(fnp.array_equal(roots[k], tree.reconstruct_root(idx, op)))
            )
    ...
    def test_reconstruct_roots_ignores_padding_levels(self) -> None:
        ...
        _, _, tree = koalabear16_merkle()
        m = fnp.arange(32, dtype=F).reshape(4, 8)  # depth 2
        root, layers = tree.commit(m)
        op = tree.open(m, layers, 2)
        max_depth = 4
        path = fnp.pad(fnp.stack(op.path), ((0, max_depth - len(op.path)), (0, 0)))
        valid = fnp.arange(max_depth) < len(op.path)
        tampered = path.at[len(op.path)].add(fnp.ones((), F))  # corrupt a pad level
        for p in (path, tampered):
            roots = tree.reconstruct_roots(
                op.row[None], fnp.array([2]), p[None], valid[None]
            )
            self.assertTrue(bool(fnp.array_equal(roots[0], root)))
    ...
    def test_single_row_root_is_its_leaf_digest(self) -> None:
        sponge, _, tree = koalabear16_merkle()
        matrix = fnp.arange(8, dtype=F).reshape(1, 8)  # height 1
        raw_root, layers = tree.commit(matrix)
        self.assertEqual(len(layers), 1)
        self.assertTrue(bool(fnp.array_equal(raw_root, sponge.hash(matrix[0]))))
    ...
    def test_commit_deterministic(self) -> None:
        ...
        self.assertTrue(bool(fnp.array_equal(r1, r2)))
    ...
    def test_non_power_of_two_height_raises(self) -> None:
        _, _, tree = koalabear16_merkle()
        matrix = fnp.arange(24, dtype=F).reshape(3, 8)  # height 3, not a power of 2
        with self.assertRaises(ValueError):
            tree.commit(matrix)
    ...
    def test_non_2d_matrix_raises(self) -> None:
        _, _, tree = koalabear16_merkle()
        with self.assertRaises(ValueError):
            tree.commit(fnp.arange(8, dtype=F))  # 1-D, not a matrix
    ...
    def test_verify_rejects_tampered_row(self) -> None:
        ...
        bad = Opening(row=op.row.at[0].add(fnp.ones((), F)), path=op.path)
    ...
    def test_verify_rejects_tampered_path(self) -> None:
        ...
        bad_path[0] = bad_path[0].at[0].add(fnp.ones((), F))
    ...
    def test_open_verify_single_leaf_empty_path(self) -> None:
        ...
        sponge, _, tree = koalabear16_merkle()
        matrix = fnp.arange(8, dtype=F).reshape(1, 8)
        root, layers = tree.commit(matrix)
        op = tree.open(matrix, layers, 0)
    ...
    def test_open_rejects_out_of_range_index(self) -> None:
        ...
        for bad in (4, -1, fnp.asarray(4), fnp.asarray(-1)):
...
class KaryMerkleTreeTest(absltest.TestCase):
    ...
    def test_commit_layer_shapes_arity4(self) -> None:
        ...
        matrix = fnp.arange(128, dtype=F).reshape(16, 8)
        raw_root, layers = tree.commit(matrix)
    ...
    def test_commit_pads_incomplete_top_level_with_zero_digests(self) -> None:
        ...
        _, _, tree = koalabear16_merkle(out=4, arity=4, chunk=4)
        matrix = fnp.arange(32 * 8, dtype=F).reshape(32, 8)
        root, layers = tree.commit(matrix)
        self.assertEqual([l.shape for l in layers], [(32, 4), (8, 4), (4, 4), (1, 4)])
        zero = fnp.zeros((4,), F)
        self.assertTrue(bool(fnp.array_equal(layers[2][2], zero)))
        self.assertTrue(bool(fnp.array_equal(layers[2][3], zero)))
        for i in (0, 7, 31):
...
class ColumnMajorMerkleTreeTest(absltest.TestCase):
    ...
    def test_open_verify_roundtrip_on_leaf_major(self) -> None:
        ...
        leaf_major = fnp.arange(32, dtype=F).reshape(4, 8)
        col_root, col_layers = col_tree.commit(leaf_major.T)
        for i in range(4):
            op = col_tree.open(leaf_major, col_layers, i)
            self.assertTrue(bool(fnp.array_equal(op.row, leaf_major[i])))
    ...
    def test_non_power_of_two_num_leaves_raises(self) -> None:
        ...
        _, col_tree = self._trees()
        with self.assertRaises(ValueError):
            col_tree.commit(fnp.arange(24, dtype=F).reshape(8, 3))
