...
def _transition(bs: _BitState, ms: _MemoryState):
...
def bp_eval_core(z_row, z_index, prefix_sum, next_prefix_sum, t_matrix, num_vars):
...
def msb_first_bits(values, num_bits: int) -> np.ndarray:
...
def build_prefix_sums(row_counts):
    sums = [0]
    for h in row_counts:
...
def build_jagged_layout(row_counts, l_max, n_r, dtype):
...
def eval_jagged_mle(col_prefix_sums, z_row, z_col, z_index, *, cfg):
