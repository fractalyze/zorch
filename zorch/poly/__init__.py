...
def eval_univariate(evals: Array, x) -> Array:
    ...
    Direct form, not barycentric: barycentric divides by ``(x - node)``, which an
    ``x`` landing on a node would zero. Nodes are built per element — an iota over
    an extension dtype is unsupported."""
    d = evals.shape[0]
