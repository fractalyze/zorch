...
def composite_or_inline(
    ...
    **attrs: object,
    ...
    `lax.composite(decomposition, name)(*operands, **attrs)` so the region lowers
    to one fused kernel (or a vendor-expanded chain, for a name-routed marker).
    Absent it, runs `decomposition(*operands, **attrs)` directly: the marker is
    dropped, the result identical.

    `attrs` ride along as composite attributes — metadata for the recognizing
    emitter — and are passed to `decomposition` on both paths, since
    `lax.composite` already calls the decomposition with them when tracing; so a
    decomposition whose attrs are required keyword arguments works inline too.
    """
    if not _HAS_COMPOSITE_OP:
        return decomposition(*operands, **attrs)
    return lax.composite(decomposition, name=name)(*operands, **attrs)
