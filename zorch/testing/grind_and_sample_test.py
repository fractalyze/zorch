...
# Both device rows, named once: the fused pairs are asserted on each.
_Transcript = Blake3FieldTranscript | Sha256FieldTranscript
_NewTranscript = Callable[[], _Transcript]
...
def _seeded(new: _NewTranscript) -> _Transcript:
...
def _state(t: _Transcript) -> list[bytes]:
...
class FusedPairTest(parameterized.TestCase):
    def _assert_same(
        self,
        unmerged: tuple[_Transcript, Array],
        merged: tuple[_Transcript, Array],
        what: str,
    ) -> None:
        t_a, r_a = unmerged
        t_b, r_b = merged
        self.assertEqual(
            fnp.asarray(r_a).tobytes(),
            fnp.asarray(r_b).tobytes(),
            f"challenge differs for {what}",
        )
        # strict: a fused pair that changed the state tree's SHAPE would
        # otherwise truncate to the shorter side and pass on the leaves it
        # did compare -- the one failure this test exists to catch.
        for i, (x, y) in enumerate(zip(_state(t_a), _state(t_b), strict=True)):
    ...
    def test_grind_and_sample_matches_unmerged(
        self, new: _NewTranscript, bits: int
    ) -> None:
        t0 = _seeded(new)
    ...
    def test_observe_scalar_and_sample_matches_unmerged(
        self, new: _NewTranscript, n: int
    ) -> None:
