...
Wraps a decomposition in a `jax.lax.composite` named `zorch.round`; zkx's
`ZorchRoundRewriter` turns that marker into a single custom-fusion kernel — one
kernel by construction, not by a per-hash compiler pattern match.
