...
def _rational_sum(numerators: Array, denominators: Array) -> Array:
    ...
    return fnp.sum(numerators / denominators)
...
class CircuitTest(absltest.TestCase):
    ...
    def test_layer_transition_folds_adjacent_pairs(self) -> None:
        ...
        ones = fnp.ones((4,), KB)
        layer = GkrLayer(
            numerator_0=fnp.array([1, 2, 3, 4], KB),
            numerator_1=fnp.array([5, 6, 7, 8], KB),
            denominator_0=ones,
            denominator_1=ones,
            num_batch_variables=1,
        )
    ...
    def test_rational_sum_invariant_end_to_end(self) -> None:
        ...
        n0 = fnp.array([3, 7, 11, 2, 5, 9, 13, 4], KB)
        n1 = fnp.array([1, 6, 8, 10, 12, 14, 15, 17], KB)
        d0 = fnp.array([2, 3, 5, 7, 11, 13, 17, 19], KB)
        d1 = fnp.array([23, 29, 31, 37, 41, 43, 47, 53], KB)
        first = GkrLayer(n0, n1, d0, d1, num_batch_variables=1)

        leaf_sum = _rational_sum(fnp.concatenate([n0, n1]), fnp.concatenate([d0, d1]))
