...
def _row_inputs(
    ...
    pair_index, eq_int, scalars; `consts` closed over. Built from a random jagged
    layer + the host round schedule, mirroring the export test's round-1 setup so
    the gather/index shapes stay self-consistent over a genuinely jagged size."""
    consts = _InterpConsts(*_round_interp_constants(KB))
    layer = random_jagged_layer(seed, _COUNTS)
    planes0 = _Planes(
        layer.numerator_0, layer.numerator_1, layer.denominator_0, layer.denominator_1
    )
    one = jnp.ones((), KB)
    rng = np.random.default_rng(seed + 99)
    z_row = jnp.asarray(rng.integers(0, _PRIME, (_NRV,), np.uint32)).view(KB)
    z_int = jnp.asarray(rng.integers(0, _PRIME, (2,), np.uint32)).view(KB)
    ...
    gather0, col0, pair0 = _round_metadata(_COUNTS, _NRV)[0]
