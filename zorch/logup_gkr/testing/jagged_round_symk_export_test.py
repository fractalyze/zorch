...
class DenseInteractionExportTest(absltest.TestCase):
    ...
    def _ef(self, shape):
        ...
        return jnp.asarray(self.rng.integers(0, _PRIME, (n, 4), np.uint32)).view(EF).reshape(shape)
    ...
    def _assert_one_binary(self, fn, multiple, has_alpha, gs):
        consts = self.consts
        ...
        if has_alpha:
            ...
            exported = export.export(jax.jit(lambda p, e, a, s: fn(p, e, a, s, consts)))(*abst)
        else:
            abst = (planes_abst, sds_m, scalars_abst)
            exported = export.export(jax.jit(lambda p, e, s: fn(p, e, s, consts)))(*abst)
        for gg in gs:
            mm = multiple * gg
            planes, eq, scalars = self._planes(mm), self._ef((mm,)), self._scalars()
            args = (planes, eq, self._ef(()), scalars) if has_alpha else (planes, eq, scalars)
            self.assertTrue(_byte_eq(fn(*args, consts), exported.call(*args)), f"m={mm}")
...
class JaggedRowExportTest(absltest.TestCase):
    ...
    def _scalars(self, seed):
        rng = np.random.default_rng(seed)
        return _RoundScalars(
            *(jnp.asarray(rng.integers(0, _PRIME, (), np.uint32)).view(KB) for _ in range(5))
        )
    ...
    def _round0(self, seed, counts):
        ...
        return (planes, gather, col_index, pair_index, eq_row, eq_int, self._scalars(seed))

    def _round1(self, seed, counts):
        ...
        _poly, planes = _sum_as_poly_row(planes0, g0, ci0, pi0, eq_row, eq_int, scal0, self.consts)
        ...
        return (planes, eq_row, alpha, gather, col_index, pair_index, eq_int, self._scalars(seed + 3))

    def test_sum_as_poly_row_one_binary(self) -> None:
        consts = self.consts
        # L pre-pad state, 2*p post-pad/gather, p pairs — one scope. eq_row/eq_int
        # static (2^nrv, 2^niv).
        L, p = export.symbolic_shape("L, p", constraints=["L>=4", "L<=64", "p>=1", "p<=32"])
        ...
        exported = export.export(jax.jit(lambda *a: _sum_as_poly_row(*a, consts)))(*abst)
        for seed, counts in enumerate(self.LAYOUTS):
            args = self._round0(seed, counts)
            self.assertTrue(
                _byte_eq(_sum_as_poly_row(*args, consts), exported.call(*args)),
    ...
    def test_fix_and_sum_row_one_binary(self) -> None:
        ...
        pp, p = export.symbolic_shape("pp, p", constraints=["pp>=1", "pp<=32", "p>=1", "p<=32"])
        sds_pp, sds_s = jax.ShapeDtypeStruct((2 * pp,), KB), jax.ShapeDtypeStruct((), KB)
        abst = (
        ...
        exported = export.export(jax.jit(lambda *a: _fix_and_sum_row(*a, consts)))(*abst)
        for seed, counts in enumerate(self.LAYOUTS):
