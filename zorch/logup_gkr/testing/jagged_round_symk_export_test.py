...
The kernels are Fiat-Shamir-less pure field arithmetic (the transcript sponge
runs between launches, outside the kernels), so the binary exports portably and
needs no poseidon plugin. The decidability gate is the bracket *declaration*: a halving
...
- Jagged row kernels (``_round_poly_row``/``_fix_and_sum_row``): the
...
class JaggedRowExportTest(absltest.TestCase):
    ...
    def test_fix_and_sum_row_one_binary(self) -> None:
        ...
        sds_pp, sds_s = jax.ShapeDtypeStruct((2 * pp,), KB), jax.ShapeDtypeStruct(
            (), KB
        )
        abst = (
