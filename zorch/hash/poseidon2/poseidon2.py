...
(`width`/`external_rounds`/`internal_rounds`/`alpha`), and routes to FXLA's
...
def _external_m4_attr(perm: Poseidon2) -> np.ndarray:
    ...
    (not a Python list) so it lowers to a `dense<[..]> : tensor<16xi64>` the FXLA
...
def _marker_attrs(perm: Poseidon2) -> tuple[dict[str, object], int]:
    ...
    shape rides as attributes — the FXLA recognizer's contract: the four
