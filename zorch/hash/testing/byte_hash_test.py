...
Both `Sha256` (device marker) and `HashlibSha256` (host) must produce the identical
...
class ByteHashTest(parameterized.TestCase):
    ...
    def test_fusion_flags(self) -> None:
        ...
        self.assertFalse(HashlibSha256().has_dedicated_fusion)
    ...
    def test_marker_equals_hashlib_impl(self) -> None:
        ...
        np.testing.assert_array_equal(
            np.asarray(Sha256().digest(msgs)), np.asarray(HashlibSha256().digest(msgs))
        )
    ...
    def test_value_identity(self) -> None:
        ...
        for cls in (Sha256, HashlibSha256):
            self.assertEqual(cls(), cls())
            self.assertEqual(hash(cls()), hash(cls()))
        self.assertNotEqual(Sha256(), HashlibSha256())
