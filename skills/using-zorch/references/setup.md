...
| `custom op 'stablehlo.composite' is unknown` on compile | The installed `frx`/plugin wheels are older than what this zorch release emits. Upgrade both together: `pip install -U pyzorch 'frx[cuda12]' --extra-index-url …`. |
...
produces the `stablehlo.composite` error above.
