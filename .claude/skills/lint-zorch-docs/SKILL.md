...
The conventions enforced here live in `docs/conventions.md` (the "subsystem doc
skeleton" + WHY-not-WHAT rules) and the **two non-negotiables** in `CLAUDE.md` /
`README.md` (proving-scheme- & zkVM-agnostic; fusion by construction). When a
...
proving-scheme- and zkVM-agnostic) and a section on **fusion by construction**.
...
`poly.md#zkx-field-dtype-gotchas`. Any other doc that re-explains a limit instead
...
1. With `--fix`: defer mechanical fixes to the upstream skill. For a Z1 missing
   doc, offer to scaffold `docs/<subsystem>.md` from the skeleton (copy the
